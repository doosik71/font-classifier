...
- **곡선 위 마우스 hover**: 마우스를 곡선 근처(약 30px 이내)로 가져가면
  가장 가까운 점의 metric 이름·가로축 값(`step`/`epoch`)·값을 작은
  주석으로 띄운다. 멀어지거나 창 밖으로 나가면 사라진다.
...
- `redraw`가 전체 패널을 오류 없이 그리고, 두 번 다시 그려도 alpha twin
  축이 오른쪽에 유지되는지.
- `--x-axis epoch` 경로.
- `update_hover`가 곡선 위 점을 정확히 찾아 (라벨·x·값) 주석을 띄우고,
  픽셀 반경 밖이나 축 밖(창 이탈)에서는 숨기는지, 여러 곡선이 있는
  패널에서 가장 가까운 곡선을 고르는지(합성 event로 headless 확인).
...
| `Monitor.redraw` | 전체 패널 다시 그리기 + hover artist 재생성 (2.5절)         |
| `Monitor.update_hover` | 마우스 위치에서 가장 가까운 곡선 점의 값을 주석으로 표시 (1.5절) |
