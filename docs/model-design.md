...
- **Font head**: `style(512, L2 정규화)` → 분류기. 1차 baseline은 softmax
  cross entropy로 시작하고, 폰트 간 혼동이 크면 ArcFace/CosFace 계열
  margin-based 분류기나 supervised contrastive loss로 전환한다. 분류
  logit과 별도로 `style` embedding 자체를 보존해 추론 시 여러 glyph의
  근거를 집계하는 데 쓴다(7절).
...
폰트 분류가 margin-based로 전환되는 시점(3.6절)도 이 커리큘럼 중 지표를
보고 결정한다.

### 4.6 최적화 실무

AdamW + cosine decay, mixed precision(fp16/bf16), 그래디언트 클리핑을
기본으로 한다. Margin 기반 font 분류기를 쓸 경우 margin을 warm-up 동안
작게 시작해 점차 키운다.
...
이 기준을 통과한 뒤에야 더 큰 backbone, margin-based font classifier 같은
복잡한 기법을 추가한다.
