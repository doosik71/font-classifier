...
`data/dataset`(폰트별 PNG + `index.json`)을 모델 학습에 바로 쓸 수 있는
...
1. **폰트 단위 LRU 메모리 캐시**: 폰트 PNG(디코딩 시 폰트당 약 9.18MiB)를
   한 번 읽으면 바이트 예산 안에서 메모리에 남겨 두고 재사용한다. 학습 중
   무작위로 섞인 (폰트, 글자) 쌍을 매번 디스크에서 다시 읽고 PNG를 다시
   디코딩하면, 글자 한 칸(4KB)을 위해 9MB짜리 파일 전체를 매번 여는 셈이라
   학습 속도의 병목이 된다.
2. **메모리 안에서 즉석으로 적용하는 augmentation**: 디스크에 결과를
...
- **2부. 상세설계서** — 캐시/augmentation/라벨 계산의 설계 근거와 트레이드오프
...
`index.json`의 `id`는 1부터 시작하는 빈틈없는 연속 정수여야 하며(현재
`construct-dataset.py`가 항상 이렇게 만든다), 아니면 생성 시점에
`ValueError`를 낸다(2.4절).
...
sampler = FontGroupBatchSampler(train_ds)  # K=32 폰트 x M=8 글자 (batch-sampler.md)
...
`batch_sampler=`를 쓸 때는 `DataLoader`에 `batch_size`/`shuffle`/`sampler`를
함께 넘기면 안 된다(PyTorch가 명시적으로 금지한다) — 배치 구성과 순서
전체를 `FontGroupBatchSampler`가 대신한다. 왜 이 샘플러가 필요한지는
[batch-sampler.md](batch-sampler.md) 참고 — 요약하면, 이 데이터셋처럼
폰트가 ~3,480개나 되는 상황에서 plain `shuffle=True`는 2.9절에서 보듯
LRU 캐시를 사실상 무력화한다.

Windows에서 `num_workers>0`을 쓰려면(2.5절) 학습 스크립트를 반드시
...
| `font_label`                              | `torch.long`, 스칼라                          | `index.json`의 `id - 1` (0-based, 분류기 클래스 인덱스로 바로 사용)                                                                             |
| `cho_label` / `jung_label` / `jong_label` | `torch.long`, 스칼라                          | 초성(0~18) / 중성(0~20) / 종성(0~27, 받침 없음 포함)                                                                                            |
| `font_id`                                 | `int`                                         | `index.json`의 `id`(1-based) — 모델 출력 인덱스로 폰트를 역추적할 때 `dataset-browser.py`(`[id] 폰트이름` 표시)와 대조하기 편하도록 그대로 둔다 |
...
### 1.5 캐시 메모리 예산과 `num_workers`

```python
train_ds = FontGlyphDataset(max_cache_bytes=2 * 1024 ** 3)  # 폰트당 ~9.18MiB 기준 약 220종
```

**중요**: `DataLoader(num_workers=N)`는 워커마다 별도 **프로세스**를
띄우고, 각 프로세스가 `FontGlyphDataset`의 독립된 사본과 독립된 캐시를
갖는다(2.5절). 따라서 실제 총 메모리 사용량은 대략
`max_cache_bytes × max(1, num_workers)`다. 시스템 전체 캐시 예산을 `T`로
잡고 싶다면 `max_cache_bytes ≈ T / num_workers`로 나눠 설정한다.
...
그 글자를 지원하지 않는 빈 칸)를 모두 흰 배경(255) 그대로 남겨
둔다([construct-dataset.md](construct-dataset.md) 2.3절). 반대로
정상적으로 추출된 칸은 잉크 픽셀이 최소 하나는 있으므로, `dataset-browser.py`
와 동일한 원리로 "칸 전체가 흰색인지"만으로 유효성을 판정한다
([dataset-browser.md](dataset-browser.md) 2.4절 참고). 이 모듈은 numpy로
벡터화해 폰트 하나(2,350칸)를 한 번에 검사한다.
...
- 폰트 하나(PNG, 약 5~6MB 압축)를 열고 디코딩하는 데 실측 약 90ms가
  ...
  병렬성은 아니다(2.9절 알려진 제한사항).
- 디코딩(`_decode_font`)은 캐시를 건드리지 않는 순수 함수라 스레드 풀에서
  안전하게 병렬 호출할 수 있다. 결과를 캐시에 넣고 `_valid_index`에
  등록하는 작업(`_register_font`)은 항상 메인 스레드에서
  `pool.map`(입력 순서를 보장)의 결과를 순서대로 소비하며 수행하므로,
  `OrderedDict` 기반 LRU 캐시(2.3절)에 여러 스레드가 동시에 쓰기 접근하는
  경쟁 상태가 없다.
- 사전 스캔에서 읽은 배열은 그대로 캐시에 들어가므로(예산 안에서), 첫
  학습 스텝을 시작하기 전부터 최근에 스캔된 폰트 일부는 이미 캐시가
  데워져 있다.
- 200개 폰트마다(그리고 완료 시) `[FontGlyphDataset] scanned i/N
  font(s)...`를 콘솔에 출력해 진행 상황을 알려준다.

### 2.3 LRU 캐시 설계 (`_LRUFontCache`)

폰트 위치(`font_pos`, `entries` 리스트의 인덱스)를 키로, 디코딩된
`(2350, 64, 64)` uint8 배열을 값으로 갖는 `OrderedDict` 기반 캐시다.

- `get`: 있으면 MRU(가장 최근 사용) 위치로 옮기고 반환, 없으면 `None`.
- `put`: 새 항목을 넣고 MRU로 옮긴 뒤, 총 바이트 수(`_nbytes`)가
  `max_bytes`를 넘는 동안 가장 오래전에 쓰인 항목(`popitem(last=False)`)
  부터 내린다.
- `len(self._data) > 1` 조건 때문에 캐시에 항목이 하나만 남으면 더 이상
  내리지 않는다 — `max_bytes`가 폰트 하나(~9.18MiB)보다 작아도 무한
  루프에 빠지거나 아무것도 캐시하지 못하는 상황을 막기 위함이다. 다만
  이 경우 캐시가 사실상 무의미해지므로, `max_cache_bytes`는 최소
  수십~수백 MiB 이상으로 설정하는 것을 권장한다.

기본값 `DEFAULT_MAX_CACHE_BYTES = 400 * BYTES_PER_FONT`(약 3.85GiB)는
"폰트 약 400종을 한 번에 들고 있는다"는 의미로, [model-design.md](model-design.md)
4.3절의 "K폰트 x M글자" 배치 샘플러(예: K=8)를 여러 배치 앞서 커버하고도
남는 여유를 준다.

### 2.4 라벨 계산과 `id` 무결성 검증

- `font_label = entry["id"] - 1`: 클래스 인덱스를 `entries` 리스트 안에서의
  **위치**가 아니라 `index.json`의 `id` 필드로 직접 계산한다. 이렇게 하면
  나중에 `entries`를 정렬/필터링하는 코드가 추가되더라도 라벨이 조용히
  어긋나는 사고를 막을 수 있다.
- 위 방식이 안전하려면 `id`가 1부터 시작하는 빈틈없는 연속 정수여야
  한다(`num_font_classes = len(entries)`와 라벨 값 범위가 맞아떨어지려면
  필수). `_load_index`가 생성 시점에 이를 검증하고, 어긋나면 바로
  `ValueError`를 낸다 — 현재 `construct-dataset.py`는 항상 이 조건을
  만족하는 `index.json`을 만들지만, 수동 편집 등으로 깨질 경우 학습이
  조용히 잘못된 라벨로 진행되는 것보다 즉시 실패하는 편이 낫다.
...
### 2.5 `DataLoader(num_workers>0)`와 메모리/피클링

PyTorch는 Windows에서 `num_workers>0`이면 `spawn` 방식으로 워커
**프로세스**를 새로 띄우고, 그 프로세스에 `Dataset` 인스턴스를 pickle해
전달한다. 여기서 두 가지를 신경 썼다.

1. **캐시를 pickle하지 않는다**: `__getstate__`를 오버라이드해, 워커로
   보내는 상태에서 `_cache`를 새로 만든 빈 `_LRUFontCache`로 교체한다.
   이렇게 하지 않으면 메인 프로세스가 사전 스캔 중 데워 둔 캐시(수백
   MiB~수 GiB일 수 있음)를 워커마다 통째로 직렬화해 보내게 되어, 워커
   수만큼 불필요한 복사와 시작 지연이 생긴다.
2. **그래도 워커마다 독립된 캐시가 생긴다**: (1)로 시작 시점의 낭비는
   막았지만, 각 워커는 이후 자신에게 배정된 인덱스를 처리하면서 자신만의
   캐시를 처음부터 다시 채운다. 즉 정상 동작 중에도 총 메모리 사용량은
   `max_cache_bytes × num_workers`에 수렴한다(1.5절). `num_workers=0`
   (메인 프로세스에서 바로 처리)을 쓰면 캐시가 하나뿐이라 메모리 사용은
   `max_cache_bytes` 그대로지만, 배치 준비가 학습 스텝과 직렬화되어
   GPU가 노는 시간이 늘 수 있다 — 메모리와 처리량의 트레이드오프이므로
   가용 RAM에 맞춰 조정한다.

### 2.6 Augmentation 파이프라인
...
- **학습/검증 분할 없음**: 이 로더는 유효한 (폰트, 글자) 쌍 전체를
  ...
  구현에 포함하지 않았다 — 요청 범위가 캐싱/augmentation이었고,
...
- **plain `shuffle=True`는 캐시를 사실상 무력화한다**: `FontGlyphDataset`
  자체는 어떤 샘플링 순서로 호출되든 정확하게 동작하지만, 완전히
  무작위인 `shuffle=True`는 폰트가 ~3,480개나 되는 규모에서 배치마다
  거의 매번 새 폰트를 요구해 LRU 캐시 적중률을 0%에 가깝게 만든다. 이
  문제를 푸는 `FontGroupBatchSampler`를
  [batch-sampler.md](batch-sampler.md)에 별도로 구현했다 — 학습에는
  `shuffle=True` 대신 이 샘플러를 쓰는 것을 권장한다.
...
- **`max_cache_bytes`가 폰트 하나보다 작으면 캐시가 무의미해진다**(2.3절).
...
| 구성 요소                                                              | 역할                                                                                                                 |
| ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `AugmentConfig`                                                        | augmentation 강도/확률 설정 dataclass (2.6절)                                                                        |
| `_geometric_pad`                                                       | 기하 변환용 패딩 폭 계산 (2.7절)                                                                                     |
| `_LRUFontCache`                                                        | 폰트 단위 바이트 예산 기반 LRU 캐시 (2.3절)                                                                          |
| `_load_index`                                                          | `index.json` 로딩 + `id` 연속성 검증 (2.4절)                                                                         |
| `FontGlyphDataset`                                                     | 공개 `Dataset` 클래스 — 사전 스캔, 캐시 조회, 라벨 계산, `__getstate__` (2.2, 2.4, 2.5절)                            |
| `FontGlyphDataset.flat_indices_by_font()`                              | 폰트별 유효 샘플 인덱스 목록 — `FontGroupBatchSampler`가 사용 ([batch-sampler.md](batch-sampler.md))                 |
| `_augment` / `_apply_geometric` / `_simulate_jpeg` / `_random_erasing` | augmentation 파이프라인 각 단계 (2.6, 2.7절)                                                                         |
| `decompose_hangul_syllable`, `NUM_CHO`/`NUM_JUNG`/`NUM_JONG`           | `font_classifier/font_dataset.py`에 추가한 초/중/종성 분해 유틸리티 (2.4절)                                          |
| `FontGroupBatchSampler`                                                | `font_classifier/batch_sampler.py`의 K폰트 x M글자 배치 샘플러 — 별도 문서 [batch-sampler.md](batch-sampler.md) 참고 |
