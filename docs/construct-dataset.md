...
2. 정렬된 순서대로 폰트를 하나씩 처리한다. 폰트마다 완성형 2,350자를
   `data/scan`의 zip/jpg에서 잘라내 64x64로 정규화한 뒤,
   `data/dataset/<font_id>/<hangul_id>.png` 형태의 **개별 PNG 파일**로 저장한다.
3. 글자 추출에 5회 이상 실패한 폰트는 품질이 낮다고 보고 통째로
   건너뛴다(1.4절, 2.3절).
4. 성공한 폰트만 `data/dataset/0000/0000.png`처럼 0-based 4자리 번호 폴더에
   저장하고, 같은 폴더의 `index.json`에 번호·폰트 이름·폴더 이름 매핑을
   기록한다.

이제 데이터셋의 기준 구조는 폰트 단위 큰 PNG가 아니라 **폰트별 폴더 안에
2,350개의 글자 PNG를 두는 형태**다. 파일 수는 크게 늘지만, 각 글자를 독립
파일로 다루는 쪽이 이후 데이터 로딩과 배치 구성에서 더 단순하다는 현재
프로젝트 방침을 따른다.
...
data/dataset/
  0000/
    0000.png  # 64x64, 그레이스케일(L 모드) — font_id 0의 hangul_id 0
    0001.png
    ...
    2349.png
  0001/
...
```json
[
  { "id": 0, "font_name": "!백묵-바람과세월체B(종)", "dir": "0000" },
  { "id": 1, "font_name": "!백묵-바람과세월체B(횡)", "dir": "0001" }
]
```

- `id`와 폴더 이름의 번호는 항상 같다(`id=0` → `0000/`).
- 번호는 **건너뛴 폰트를 빼고** 저장에 성공한 순서대로 0부터 다시
  매긴다 — annotation 폰트 목록 안에서의 원래 순번이 아니다.
- 각 폰트 폴더 안의 `0000.png`~`2349.png`는 `font_classifier.font_dataset.
  HANGUL_TABLE` 순서(완성형 한글 코드값 순)를 그대로 따른다.
- 추출에 실패한 낱칸(annotation 없음, 빈 칸, 영상 로드 실패)도 파일은
  만들며, 내용은 흰색(255) 이미지다. PNG 자체에는 실패 원인이 따로
  저장되지 않으므로 필요하면 콘솔 로그(1.3절)나 원본 annotation의
  `char_pages` 커버리지로 역추적해야 한다.
...
- **재실행은 항상 처음부터 다시 만든다**: 이어하기(resume) 기능은 없다.
  다시 실행하면 폰트 목록을 처음부터 다시 정렬하고, 0번부터 번호를 다시
  매겨 `data/dataset/<font_id>/` 폴더를 다시 만들며 `index.json`도 새로
